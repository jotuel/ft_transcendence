    ...
    <div className="flex flex-col w-full min-h-screen z-0">
      <nav className="bg-slate-200">
        <Container className="flex items-center gap-2 sm:gap-4 h-14 [&>a]:text-sky-900">
          ...
          // more links here...
          */}
          <Link to="/login" className="md:absolute right-1 block py-2 px-3 font-medium no-underline hover:underline">
            Login
          </Link>
        </Container>
      </nav>
      ...
      <footer className="bg-slate-200">
        <Container className="py-4 text-center text-slate-800/75 text-sm">
          &copy; {new Date().getFullYear()}&nbsp;PongCompany. All rights reserved.
        </Container>
      </footer>
    </div>
  )
